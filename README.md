...
- `factor_size`: the size of the output (i.e., the smallest number of factors of a bidirectional macro scheme (BMS), the smallest size of a string attractor, the smallest size of a grammar)
- `factors`: an instance of a valid output attaining the size `factor_size`. This is
  ...
  * for a bidirectional macro scheme (BMS) a list of pairs [pos, len] for the direction to copy from `T[pos]` a substring of length `len`. If `pos` is -1, then the factor is a ground phrase and it stores its character in `len`.
...
### Computing minimum bidirectional macro schemes (BMS) with PySAT
